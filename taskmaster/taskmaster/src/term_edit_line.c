...
/*   Updated: 2016/05/06 11:24:47 by dbaldy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	term_spec_char(char buf, t_line_list **first)
{
	if (buf == 28)
	{
		sh_reset_term();
		ft_dprintf(STDOUT_FILENO, "\ttaskmaster: exit\n");
		exit(0);
	}
	else if (buf == 4)
...
{
	if (arr[1] == 1)
		return (term_spec_char(buf[0], first));
	else if (arr[1] == 4)
